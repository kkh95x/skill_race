...
final userAuthNotifer=StateNotifierProvider<AuthNotifer,AuthState>((ref) => AuthNotifer(ref.read(userServiceProvider),ref));



class AuthNotifer extends StateNotifier<AuthState>{
 final Ref _ref;
  AuthNotifer(this._userService,this._ref):super(const AuthState(state: AuthStatus.loading)){
...
Future<void> sginInWithGoogle()async{
  ...
  final usercredential =await _userService.signInWithGoogle();
    if(usercredential.user!=null){
      ...
      if(user!=null){
        state =AuthState(state: AuthStatus.auth,
      ...
      state =AuthState(state: AuthStatus.selectType,id: usercredential.user?.uid,email: usercredential.user?.email);
         BotToast.closeAllLoading();
...
final accountType=form.control(UserFormkeys.accountType).value as AccountType;
...
final specializtion=form.control(UserFormkeys.empSpecialization).value as Specialization;
final supSpeciaiztion=form.control(UserFormkeys.empSupSpecialization).value;
BotToast.showLoading();
final user=AppUser(fullname: name,
 accountType: accountType,
 country: coutnryName,
 city: cityName,
  id: id,
 email: email,
 employee: Employee(jobType: jobName,specialization: specializtion.name,supSpecialization: supSpeciaiztion));
...
final phone=form.control(UserFormkeys.hiringPhoneNumber).value as PhoneNumber;
...
Future<void> logOut()async{
  ...
  if(removed??false){
    BotToast.closeAllLoading();
    state=const AuthState(state: AuthStatus.unAtuth,currentUser: null,id: null);
